...
# Fixed Risk Weekly Compound Validation v1.1
#
# Save path:
...
# - Test multiple risk percentages per trade:
#   1.0% / 1.5% / 2.0%
...
def simulate_fixed_risk_weekly(df, label, risk_pct_per_trade):
    ...
    for _, row in df.iterrows():
        ...
        out['RiskPctLabel'] = f'{risk_pct_per_trade * 100:.1f}%'
        out['Week'] = week
    ...
    summary = {
        ...
        'RiskPctLabel': f'{risk_pct_per_trade * 100:.1f}%',
...
def run_all_simulations(df):
    ...
    for risk_pct in RISK_PCT_LIST:
        for (event_scenario, atr_condition), df_g in df.groupby(group_cols):
            label = f'{event_scenario} | {atr_condition} | Risk {risk_pct * 100:.1f}%'
...
summary_path = f'{OUTPUT_DIR}/FixedRiskWeeklyCompound_RiskCompare_Summary_v1_1.csv'
trades_path = f'{OUTPUT_DIR}/FixedRiskWeeklyCompound_RiskCompare_Trades_v1_1.csv'
weekly_path = f'{OUTPUT_DIR}/FixedRiskWeeklyCompound_RiskCompare_Weekly_v1_1.csv'
...
print('\n' + '=' * 80)
print('🏆 Fixed Risk Weekly Compound Risk Compare Summary v1.1')
print('=' * 80)
